  ...
  useEffect(() => {
    ...
    gsap.to(mainRef.current, {
      ...
      onComplete: () => {
        ...
        gsap.fromTo(mainRef.current, { opacity: 0, x: 20 }, { opacity: 1, x: 0, duration: 0.5 })
      },
    })
          ...
          <div ref={mainRef} className="relative rounded-lg overflow-hidden mb-6">
            <div className="relative h-[400px] bg-gradient-to-r from-gray-300/10 to-gray-100/5">
              ...
              <div className="relative z-10 p-8 text-white h-full flex flex-col justify-center max-w-2xl">
                <h2 className="text-3xl font-bold mb-1">{testimonials[activeIndex].name}</h2>
                <p className="text-lg mb-6">{testimonials[activeIndex].position}</p>

                <p className="text-sm leading-relaxed mb-6">{testimonials[activeIndex].text}</p>
          ...
          {testimonials.map((testimonial, index) => (
            <div
              ...
              className={`relative rounded-lg overflow-hidden cursor-pointer border-2 transition-all ${
                activeIndex === index ? "border-[#3AC29A]" : "border-transparent"
              }`}
            >
              <div className="relative h-[150px]">
                ...
                <div className="absolute bottom-0 left-0 p-3 text-white">
                  ...
                  <div className="flex mt-1">
                    {[...Array(5)].map((_, i) => (
                      <svg
                        key={i}
                        className={`w-3 h-3 ${i < testimonial.rating ? "text-yellow-400" : "text-gray-400"}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    gsap.fromTo(e.currentTarget, { scale: 0.8 }, { scale: 1, duration: 0.3, ease: "back.out(1.7)" })
                  }}
                >
